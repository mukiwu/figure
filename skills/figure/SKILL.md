...
- Preserve the rabbit's character proportions in every final prompt: small slim rabbit, rounded head only slightly larger than torso, narrow shoulders, tiny hips, thin limbs, compact lightweight silhouette. Do not make the rabbit stocky, bulky, muscular, thick-limbed, giant-headed, top-heavy, babyish, or chibi.
...
- Keep text labels sparse, short, legible, and embedded in the diagram as handwritten annotations. Prefer 2-5 labels per image.
...
- Use moderate notebook-style annotations by default: a few small arrows, two circled checkpoints at most, one bracket mark when useful, and very sparse side notes. The information density should sit between a clean flowchart and a busy notebook page: visibly annotated, but still airy and easy to read.
- Use this label style exactly: 中文標註風格：不工整的手寫感中文字，細線條、自然抖動、字距和大小略不一致，像快速用細黑筆寫在筆記本旁邊；每一行文字維持水平齊平，不往右上角傾斜，不旋轉；字形鬆弛、有文青筆記感，但必須保持清楚可讀。
...
6. Generate images:
   ...
   - Ask image-2 to render the 2-5 text labels directly using the default handwritten notebook label style and the source language rules.
